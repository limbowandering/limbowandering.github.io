...
date: 2017-10-23
categories: Web
tags: [Web, 原则, 代码, Sublime]
---

上次在课上小组写代码时候, 曹倡突然说: "你竟然是TAB党!" 额, 其实我是soft tab党. 不过有考虑转成空格党了.

不过, 这也确实让我看重了一下代码规范问题, 作为个人, 应该有自己习惯性的代码风格, 作为团队, 应保证代码规范. 先从自己做起, 有了团队再跟团队规范.

参考 [AlloyTeam](http://alloyteam.github.io/CodeGuide) [菜鸟教程](http://www.runoob.com/w3cnote/html-css-guide.html#html-javascript) 

## 命名规则

- 项目命名: 全部小写, 以下划线分隔. my_project_name
- 目录命名:  同项目命名, 有复数结构时候用复数命名法. scripts, date_models
- JS文件命名: 同项目命名, account_model.js
- CSS文件命名: 同项目命名.
- HTML文件命名: 同项目命名.
...
- 在属性上，使用双引号，不要使用单引号；
- 属性名全小写，用中划线做分隔符；
- 不要在自动闭合(self-closing)标签结尾处使用斜线（[HTML5 规范](http://dev.w3.org/html5/spec-author-view/syntax.html#syntax-start-tag)指出他们是可选的）；
...
- 命名: 类名使用小写字母，以中划线分隔; id采用驼峰式命名;scss中的变量、函数、混合、placeholder采用驼峰式命名

- 颜色;颜色16进制用小写字母；颜色16进制尽量用简写。
...
- SCSS相关: 提交的代码中不要有 `@debug`；声明顺序:`@extend`
...
soft tab(4个空格) 但是推荐敲空格. 因为不同编辑器对TAB键的解析不一样
